...
def create_ecsv_table(schedule_list: list[dict]):
    ...
    -------
    astropy.table.Table
        Table with columns and metadata suitable for ECSV format
    """
...
def save_schedule_to_ecsv(schedule_list, output_file):
    ...
    schedule_list : list of dict
        List of observation blocks from the scheduler
    output_file : str
        Path to save the ECSV file
        
    Returns
    -------
    astropy.table.Table
        The table that was saved to disk
    """
...
def table_to_blocks(table):
    """
    Convert an ECSV table back to a list of observation blocks
    
    Parameters
    ----------
    table : astropy.table.Table
        ECSV table containing schedule data
        
    Returns
    -------
    list of dict
        List of observation blocks that can be used with the scheduler
    """
    blocks = []
    ...
    for row in table:
        ...
        for colname in table.colnames:
            block[colname] = row[colname]
            
        # Convert string coordinates to SkyCoord objects
        if 'target_ra' in block and 'target_dec' in block:
            block['target'] = SkyCoord(block['target_ra'], block['target_dec'], unit='deg')
            
        # Add proper units to appropriate fields
        if 'duration' in block:
            block['duration'] = block['duration'] * u.second
            
        if 'pm_ra_cosdec' in block:
            block['pm_ra_cosdec'] = block['pm_ra_cosdec'] * u.arcsec/u.hour
            
        if 'pm_dec' in block:
            block['pm_dec'] = block['pm_dec'] * u.arcsec/u.hour
            
        # Convert comma-separated observers back to list
        if 'observer' in block and isinstance(block['observer'], str):
            block['observer'] = block['observer'].split(',')
